...
const UserVideoComponent = ({ streamManager, classVideo, classVideoCircleCropper }: MyProps) => {
  ...
  const ThumbCaption = (): any => {
    ...
    if (classVideo === 'video-circle' && streamManager?.openvidu?.role === 'PUBLISHER') {
      return (
        <div>
          <p className="caption text-lightGrey text-center">You</p>
        </div>
      );
    }
    return (
      <div>
        <p className="caption text-lightGrey text-center overflow-ellipsis overflow-hidden whitespace-nowrap">
          {getNicknameTag()}
        </p>
      </div>
    );
